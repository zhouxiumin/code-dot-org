...
When(/^I open my eyes to test "([^"]*)"$/) do |test_name|
  ...
  @eyes.branch_name = ENV['EYES_BRANCH'] || GitUtils.current_branch

  pr_base = GitUtils.circle_pr_branch_base_no_origin
  if pr_base
    puts "Branch is #{pr_base}"
    @eyes.parent_branch_name = ENV['EYES_PARENT'] || pr_base
  else
    ...
    @eyes.parent_branch_name = ENV['EYES_PARENT'] || fallback_branch
  end
