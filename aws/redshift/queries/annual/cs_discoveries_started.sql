-- Distinct teacher-student combinations who started CSD in the first half of 2017-18 school year
drop table if exists #csd_first_half;
create temp table #csd_first_half as
...
where sc.id in (select script_id from course_scripts cs join courses c on c.id = cs.course_id where c.name = 'csd')
